...
const InventoryList = () => {
    ...
    const categoryAverages = categories.map((category) => {
      ...
      const categoryAverage =
        categoryPrices.length > 0
          ? categoryPrices.reduce((acc, price) => acc + price, 0) /
            categoryPrices.length
          : 0;
      return {
        category: category.name,
        averagePrice: categoryAverage.toFixed(2),
      };
    });
  ...
  const handleAddItem = async (newItem) => {
    try {
      ...
      const barcodeExists =
        items.some((item) => item.barcode === newItem.barcode) ||
        items.some(
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesCategory =
      filterCategory === "all" ||
      item.category_id === parseInt(filterCategory, 10);
    const matchesStatus =
      filterStatus === "all" ||
            ...
            {isEditing === item.id ? (
              ...
              <>
                ...
                >
                  ...
                  <span className="inventory-item-sellPrice">
                    ${item.sellPrice}
                  </span>
                </div>
                {isAdmin && (
