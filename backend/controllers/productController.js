  ...
  try {
    ...
    db.query(checkBarcode, [barcode], (checkErr, checkResult) => {
      ...
      if (checkResult.length > 0) {
        return res
          .status(400)
          .json({ error: 'An item with this barcode already exists' });
      }

      // If barcode doesn't exist, proceed with insertion
      const sql =
        'INSERT INTO items (name, quantity, barcode, price, sellPrice, category_id) VALUES (?, ?, ?, ?, ?, ?)';
      db.query(
        sql,
        [name, quantity, barcode, price, sellPrice, category_id],
        (err, result) => {
          if (err) {
            return res.status(500).json({ error: 'Database error' });
          }
...
const updateProduct = async (req, res) => {
  ...
  try {
    const query =
      'UPDATE items SET name =?, quantity = ?, price = ?, sellPrice = ?, category_id = ? WHERE id = ?';
    db.query(
      query,
      [name, quantity, price, sellPrice, category_id, id],
      (err, result) => {
        if (err) {
...
const processCheckout = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const [pointSettings] = await db
        .promise()
        .query('SELECT * FROM point_settings ORDER BY id DESC LIMIT 1');
      const {
        points_per_amount,
        discount_per_point,
      ...
      if (customer_id) {
        ...
        if (use_points) {
          const [customerResults] = await db
            .promise()
            .query('SELECT points FROM customers WHERE id = ?', [customer_id]);
          const customer = customerResults[0];

          if (customer.points >= minimum_points_for_discount) {
            const maxDiscount =
              Math.floor(customer.points / minimum_points_for_discount) *
              discount_per_point;
            discount = Math.min(maxDiscount, total_amount);
            pointsRedeemed =
              Math.floor(discount / discount_per_point) *
              minimum_points_for_discount;
...
const getOrders = async (req, res) => {
  ...
  try {
    ...
    const params = isAdmin ? (userFilter ? [`%${userFilter}%`] : []) : [userId];
      ...
      try {
        ...
        db.query(getPointsQuery, [id], async (err, results) => {
          ...
          if (points_to_redeem < customer.minimum_points_for_discount) {
            return res
              .status(400)
              .json({ message: 'Minimum points requirement not met' });
          }

          const discount_amount =
            points_to_redeem * customer.discount_per_point;
