...
const getUser = async (req, res) => {
  try {
    const query =
      'SELECT * FROM user_table WHERE isAdmin = 0 AND is_active = TRUE';
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (result.affectedRows === 0) {
          return res
            .status(404)
            .json({ error: 'User not found or cannot update admin' });
        }
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (result.affectedRows === 0) {
          return res
            .status(404)
            .json({ error: 'User not found or cannot delete admin' });
        }
